...
| lora | нет | 16.85% ± 1.37 | 16.91% ± 1.35 | 91 |

lora обучен трижды (сиды 0/1/2): EX 15.38/18.68/16.48 → среднее **16.85 ± 1.37**; VES 15.75/18.40/16.59
→ **16.91 ± 1.35**. baseline детерминирован (greedy). Разрыв 19.78 − 16.85 = **2.93 п.п. по EX, больше
2σ — реален**: на этой конфигурации схема в контексте пока устойчиво лучше схемы в весах на ~3 п.п.
**VES ≈ EX везде** — корректные запросы исполняются с той же скоростью, что gold, так что эффективность
армы не различает. Тезис ветки — не «догнали baseline», а «3B вообще удерживает схему трёх баз в весах,
отставая контролируемо». Экономия токенов (288 → 91) реальна, но у локальной 3B почти ничего не стоит —
см. размеры схем, продавать её нельзя.
