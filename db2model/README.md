...
| **qwen27b_sql_only** (1 сид) | **7B** | нет | **36.26%** | **90** |

**Знание в весах обгоняет знание в контексте — на обоих размерах модели:**
- **7B** (плановая основная): lora **36.26%** vs baseline 28.57% = **+7.7 п.п.** при контексте в 3.2× меньше. Лучшее число, выше baseline напарницы (24.18) на +12.
- **3B**: lora 29.30% ± 1.37 (сиды 30.77/27.47/29.67) vs baseline 19.78% = **+9.5 п.п.**, разрыв ≫ σ.

7B — один сид (число сильное, но для строгости нужны сиды; на 3B единичный прогон давал
32.97% — высокий сид, честное среднее 29.30). Адаптеры — `adapters/qwen27b_7b_sqlonly1096/`
(7B) и `qwen27b_sqlonly1096/` (3B); данные — `kaggle_input/` (sql_only; мультизадачи дали
−4 п.п. и отключены, воспроизводятся флагом `build_dataset.py --multitask`).
