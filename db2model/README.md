...
| Арм (lora = знание в весах) | Схема в промпте | EX | prompt-токенов |
|---|---|---|---|
| zeroshot (контроль) | нет | 4.40% | 90 |
| baseline | да | 19.78% | 288 |
| baseline напарницы (light-RAG инструмент) | — | 24.18% | — |
| **qwen27b_sql_only** (3 сида) | нет | **29.30% ± 1.37** | **90** |

**Знание в весах обгоняет знание в контексте: +9.5 п.п. над baseline (19.78) при контексте
в 3.2× меньше** (90 vs 288 токенов, разрыв ≫ σ), и выше baseline напарницы (24.18) на +5.1.
Сиды 0/1/2: 30.77 / 27.47 / 29.67. Одиночный прогон дал 32.97% — **высокий сид**, честное
число — среднее 29.30 ± 1.37 (та же поправка, что была с seed42). Победивший адаптер —
`adapters/qwen27b_sqlonly1096/`, данные — `kaggle_input/` (канонический = sql_only;
мультизадачный вариант — флаг `build_dataset.py --multitask`, дал −4 п.п., отключён).
