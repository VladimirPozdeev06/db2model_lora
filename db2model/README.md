...
| **qwen27b_sql_only** | нет | **32.97%** | **90** |

**Знание в весах обгоняет знание в контексте: +13.2 п.п. над baseline (19.78) при контексте
в 3.2× меньше** (90 vs 288 токенов), и выше baseline напарницы (24.18). Чистый ablation
(один оценщик): сильный учитель 7B→27B **+5.5 п.п.**, мультизадачность ROUTE **−7.7 п.п.**
(её отключили). Победивший адаптер — `adapters/qwen27b_sqlonly1096/`, данные — `kaggle_input/`
(канонический = sql_only; мультизадачный вариант воспроизводится флагом `build_dataset.py --multitask`).
