...
RESULTS = ROOT / "db2model" / "results"
GOLD = ROOT / "data" / "bird_large_filtered.json"
...
ARMS: list[tuple[str, str, str, list[str]]] = [
    ...
    ("canon", "qwen27b_sql_only (3 сида)", "3B", [
    ...
    ("canon", "qwen27b_sql_only (3 сида)", "7B", [
...
def main() -> int:
    ...
    ap.add_argument("--out", default="", help="куда сохранить JSON с числами")
    args = ap.parse_args()
    ...
    for group, label, target, files in ARMS:
        ...
        row = {
            "group": group, "arm": label, "target": target, "seeds": len(reps),
            "ex_mean": statistics.fmean(ex), "ex_sigma": pstdev(ex), "ex_all": ex,
            "ves_mean": statistics.fmean(ves), "ves_sigma": pstdev(ves),
