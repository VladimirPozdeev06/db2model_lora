...
# Schema-less variant for the "knowledge in weights" (LoRA) arm: the target model
# was fine-tuned on this specific database, so the schema is NOT injected into the
# prompt — it is expected to come from the adapter weights. This is what makes the
# token budget of the LoRA arm small. See db2model/MCP_SERVING.md (case B).
# Schema-less арм (знание о БД — в весах LoRA-адаптера).
#
# ВНИМАНИЕ: это дословно та инструкция, на которой адаптер обучался
# (`db2model/kaggle_train_*.ipynb`, функция `build_prompt`) и на которой снимались
# все числа лидерборда. Адаптер — узкий артефакт: он выучил отвечать именно на неё.
# Любое «улучшение» — лишние правила, другой язык, требование комментариев — уводит
# модель с обученного распределения, и она перестаёт выдавать SQL вообще.
